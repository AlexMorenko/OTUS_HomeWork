...
SET @StrQuery = 
N';WITH CustomerPurchase AS ( SELECT c.CustomerName, o.OrderDate,COUNT(*) AS OrderCount
FROM Sales.Orders o
INNER JOIN Sales.OrderLines ol ON o.OrderID = ol.OrderID
INNER JOIN Sales.Customers c ON o.CustomerID = c.CustomerID
GROUP BY o.OrderDate, c.CustomerName
)
SELECT p.OrderDate, ' + @StrSelect +
' FROM ( SELECT * FROM CustomerPurchase cp) t
PIVOT (	SUM(OrderCount)	FOR CustomerName IN ( ' + @StrSelect + ' )
) p ORDER BY p.OrderDate';
