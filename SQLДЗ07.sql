...
;WITH CustomerItem AS ( 
 SELECT  CAST(FORMAT(OrderDate,'01.MM.yyyy') AS DATE)  AS OrderDate, c.CustomerName, 	COUNT(*) AS OrderCount
   FROM Sales.Orders o
		INNER JOIN Sales.OrderLines ol ON o.OrderID = ol.OrderID
		INNER JOIN Sales.Customers c ON o.CustomerID = c.CustomerID
   WHERE o.CustomerID IN (2, 3, 4)
   GROUP BY FORMAT(OrderDate,'01.MM.yyyy') , c.CustomerName
)
...
FROM  CustomerItem  t PIVOT (SUM(OrderCount)
	 FOR CustomerName IN ([Tailspin Toys (Medicine Lodge, KS)]
...
SELECT   c.CountryID ,c.CountryName ,c2.Code
...
SELECT  c.CustomerID ,c.CustomerName ,ool.StockItemID ,ool.UnitPrice ,ool.OrderDate
FROM Sales.Customers c
CROSS APPLY (SELECT TOP 2     o.OrderDate   ,ol.UnitPrice   ,ol.StockItemID
			  FROM Sales.Orders o   INNER JOIN Sales.OrderLines ol
				ON o.OrderID = ol.OrderID
				WHERE o.CustomerID = c.CustomerID
				ORDER BY ol.UnitPrice DESC) ool
